...
jupyter:
  ...
  kernelspec:
    ...
    language: R
    name: ir
title: 'Dataset: Exposure to particulate matter less than 2.5 microns in diameter'
---
...
The data was collected across 107 countries, which have been grouped into 7 categories by the World Health Organization (WHO),
and further grouped by a hierarchical clustering of ground monitor measurements.
...
Function `standata` retrieves the input data.
...
```{r}
stan_complete_pool_data_asis = stancode(fit_complete_pool_data_asis)
cat(stan_complete_pool_data_asis)
```

**Takeaway**   zero-centered data is easier to sample!!!
...
Before fitting to the observed data, we check what data our priors imply by sampling from the prior alone (`sample_prior = "only"`). 
`pp_check(..., prefix = "ppd")` plots the prior predictive distribution (the simulated outcomes, without the observed data).

Recall the outcome is on the *log* scale. Vague priors (sometimes recommended as "non-informative") generate implausible data:

```{r}
fit_prior_vague = brm(log_pm25 ~ 1 + log_sat, data=sites,
    prior = c(
      prior(normal(0, 100), class = Intercept),
      prior(normal(0, 100), class = b),
      prior(normal(0, 100), class = sigma)),
    sample_prior = "only", cores=4, refresh=0, seed=234098)
```

```{r}
pp_check(fit_prior_vague, prefix = "ppd", ndraws = 100)
```

On the log scale these simulated readings span many orders of magnitude (and are physically impossible).
The "non-informative" priors are in fact strongly informative on the data scale.

Weakly informative priors, centred near intercept 0 and slope 1 generate more plausible data:

```{r}
fit_prior_wi = brm(log_pm25 ~ 1 + log_sat, data=sites,
    prior = c(
...
```{r}
pp_check(fit_prior_wi, prefix = "ppd", ndraws = 100)
```

These priors are still only weakly informative: 
the implied data-generating process can still produce values more extreme than domain knowledge would suggest, but it no longer puts mass on impossible data.
...
fit_super_region = brm(log_pm25 ~ 1 + log_sat + (1 + log_sat | super_region_name),
                       ...
                       max_treedepth=12, adapt_delta=0.9, refresh=100, seed=234098)
```
...
```{r}
print(make_stancode(log_pm25 ~ 1 + log_sat + (1 + log_sat | super_region_name), data=sites))
```
...
```{r}
loo_compare(loo_complete, loo_super_region, loo_cluster_region)
```
